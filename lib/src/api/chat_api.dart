...
class ChatApi {
  Future<Conversation> initiateChat({required String recipientUserId, String? orderId}) async {
    try {
  ...
  Future<Map<String, dynamic>> getMessages(String chatId, {int page = 1, int limit = 50}) async {
    try {
      final response = await ApiClient.dio.get('/chats/$chatId/messages', queryParameters: {
        'page': page,
        'limit': limit,
      });
  ...
  Future<void> markRead(String chatId) async {
    try {
      ...
      throw Exception(e.response?.data['error'] ?? 'Failed to mark chat as read');
    }
  }
}
